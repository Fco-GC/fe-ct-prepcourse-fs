...
function incrementarPorUno(array) {
   ...
   // Tu código:
   incrementedArray = array.map(n => n + 1);
...
function promedioResultadosTest(resultadosTest) {
   ...
   var mean = add / resultadosTest.length;
   return mean;
}
...
function numeroMasGrande(arrayOfNums) {
   ...
   let bigger = 0;
   for (let i = 0; i < arrayOfNums.length; i++){
      if (arrayOfNums[i] > bigger) bigger = arrayOfNums[i]
   }
   return bigger;
}
...
function multiplicarArgumentos() {
   ...
   // arguments es una palabra reservada que permite pasar un número indefinido de argumentos a una función.
